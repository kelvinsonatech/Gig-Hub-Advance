...
/**
 * Creates tables (if not exist) and seeds essential data on first deploy.
 * Safe to call on every startup — all operations are idempotent.
 */
export async function seedDatabase() {
  if (!pool) {
    logger.warn("[seed] No DB pool — skipping seed");
    return;
  }
  ...
  try {
    ...
    await client.query(`
      INSERT INTO networks (name, code, color, logo_url, tagline) VALUES
        ('MTN Ghana',     'MTN',     '#FFCC00',
          'https://upload.wikimedia.org/wikipedia/commons/thumb/9/93/New-mtn-logo.jpg/500px-New-mtn-logo.jpg',
          'Ghana''s Largest Network'),
        ('AirtelTigo',    'AT',      '#004b87',
          'https://recharge-prd.asset.akeneo.cloud/product_assets/media/recharge_com_airteltigo_product_card.png',
          'Connecting Communities'),
        ('Telecel Ghana', 'TELECEL', '#CC0000',
          'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRJQ6fNzCpMhlyaxWqlXYqmY7Bb5KZBIQt_1Q&s',
          'Fast & Reliable Data')
    ...
    // ── Bundles — only seed when table is empty ────────────────────────────
    const { rows } = await client.query(`SELECT COUNT(*) AS c FROM bundles`);
    const count = parseInt(rows[0].c, 10);

    if (count === 0) {
      await client.query(`
        INSERT INTO bundles (network_id, network_name, name, data, validity, price, type, popular)
        SELECT n.id, 'MTN Ghana', v.bname, v.data, v.validity, v.price::NUMERIC, v.type, v.popular::BOOLEAN
        FROM networks n,
             (VALUES
             ...
             ) AS v(bname, data, validity, price, type, popular)
        WHERE n.code = 'MTN';

        INSERT INTO bundles (network_id, network_name, name, data, validity, price, type, popular)
        SELECT n.id, 'AirtelTigo', v.bname, v.data, v.validity, v.price::NUMERIC, v.type, v.popular::BOOLEAN
        FROM networks n,
             (VALUES
        ...
        INSERT INTO bundles (network_id, network_name, name, data, validity, price, type, popular)
        SELECT n.id, 'Telecel Ghana', v.bname, v.data, v.validity, v.price::NUMERIC, v.type, v.popular::BOOLEAN
        FROM networks n,
             (VALUES
      ...
      `);
      logger.info("[seed] ✓ 16 bundles seeded across 3 networks");
    } else {
      logger.info(`[seed] Bundles already present (${count}) — skipping`);
    }

    logger.info("[seed] ✓ Database ready");
  } catch (err: any) {
    logger.error({ err: err.message }, "[seed] Failed — continuing startup anyway");
  } finally {
    client.release();
  }
}
